...
contract TimelockController is AccessControl {
    ...
    uint128 private _minDelay;
    uint128 private _maxDelay;
    ...
    constructor(
        ...
        _minDelay = minDelay;
        _maxDelay = maxDelay;
        emit DelayChange(0, minDelay, 0, maxDelay);
    }
     ...
     */
    function getDelay() public view virtual returns (uint128, uint128) {
    ...
    function _schedule(bytes32 id, uint256 delay) private {
        ...
        (uint128 minDelay, uint128 maxDelay) = getDelay();
        require(
            delay >= uint256(minDelay) && delay <= uint256(maxDelay),
     ...
     */
    function updateDelay(uint128 newMinDelay, uint128 newMaxDelay) external virtual {
        require(
            msg.sender == address(this),
            "TimelockController: caller must be timelock"
        );
        emit DelayChange(_minDelay, newMinDelay, _maxDelay, newMaxDelay);
