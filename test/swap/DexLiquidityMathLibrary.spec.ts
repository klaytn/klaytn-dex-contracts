...
describe('DexComputeLiquidityValue', () => {
  ...
  beforeEach('mint some liquidity for the pair at 1:100 (100 shares minted)', async () => {
    ...
    await token1.transfer(pair.address, ethers.utils.parseEther('1000'));
    await pair.mint(wallet.address);
    expect(await pair.totalSupply()).to.eq(ethers.utils.parseEther('100'));
  });
  ...
  describe('#getLiquidityValue', () => {
    ...
    describe('fee on', () => {
      ...
      beforeEach('mint more liquidity to address zero', async () => {
        ...
        expect(await pair.totalSupply()).to.eq(ethers.utils.parseEther('200'));
      });
  ...
  describe('#getLiquidityValue', () => {
    describe('fee is off', () => {
      it('produces the correct value after arbing to 1:105', async () => {
        const [token0Amount, token1Amount] = await computeLiquidityValue
