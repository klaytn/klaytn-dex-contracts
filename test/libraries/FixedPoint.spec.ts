...
describe('FixedPoint', () => {
  ...
  before('deploy FixedPointTest', async () => {
    const fixedPointFactoty = await ethers.getContractFactory('FixedPointTest');
    fixedPoint = await fixedPointFactoty.deploy();
  });
  ...
  describe('#encode144', () => {
    ...
    it('will not take >uint144(-1)', async () => {
      expect(() => fixedPoint.encode144(
        BigNumber.from(2).pow(145).sub(1),
      )).to.throw;
    });
  });
  ...
  describe('#decode', () => {
    ...
    it('will not take >uint224(-1)', async () => {
      expect(() => fixedPoint.decode(
        [BigNumber.from(2).pow(225).sub(1)],
      )).to.throw;
    });
  });
  ...
  describe('#decode144', () => {
    ...
    it('will not take >uint256(-1)', async () => {
      expect(() => fixedPoint.decode(
        [BigNumber.from(2).pow(257).sub(1)],
      )).to.throw;
    });
  });
  ...
  describe('#mul', () => {
    ...
    it('overflow', async () => {
      await expect(fixedPoint.mul(
        [BigNumber.from(1).mul(Q112)],
        BigNumber.from(2).pow(144),
      )).to.be.revertedWith(
  ...
  describe('#muli', () => {
    it('works for 0', async () => {
      expect(await fixedPoint.muli(
        [BigNumber.from(0).mul(Q112)],
        BigNumber.from(1),
    ...
    it('max without overflow, largest int', async () => {
      ...
      expect(await fixedPoint.muli([BigNumber.from(1).mul(Q112)], minInt.add(1)))
        .to.be.eq(minInt.add(1));
    });
    it('max without overflow, largest fixed point', async () => {
  ...
  describe('#muluq', () => {
    ...
    it('multiplies 3*2', async () => {
      expect((await fixedPoint.muluq(
        [BigNumber.from(3).mul(Q112)],
        [BigNumber.from(2).mul(Q112)],
      ))[0]).to.eq(
        BigNumber.from(3).mul(2).mul(Q112),
      );
    });
  ...
  describe('#divuq', () => {
    ...
    it('divides 35/8', async () => {
      expect((await fixedPoint.divuq(
        [BigNumber.from(35).mul(Q112)],
        [BigNumber.from(8).mul(Q112)],
      ))[0]).to.eq(
    ...
    it('boundary of full precision', async () => {
      ...
      expect((await fixedPoint.divuq(
        [maxNumeratorFullPrecision],
        [minDenominatorFullPrecision],
      ))[0]).to.eq(
    ...
    it('gas cost of full precision small dividend short circuit [ @skip-on-coverage ]', async () => {
      ...
      expect(await fixedPoint.getGasCostOfDivuq([BigNumber.from(28).mul(Q112)], [BigNumber.from(280)
        .mul(Q112)])).to.eq(
        959,
      );
      expect(await fixedPoint.getGasCostOfDivuq([BigNumber.from(1).mul(Q112)], [BigNumber.from(3)
        .mul(Q112)])).to.eq(
        959,
      );
    });
  ...
  describe('#fraction', () => {
    ...
    it('can be called with numerator exceeding uint112 max', async () => {
      expect((await fixedPoint.fraction(Q112.mul(2359), 6950))[0])
        .to.eq(Q112.mul(Q112).mul(2359).div(6950));
    });
    ...
    it('can be called with numerator exceeding uint144 max', async () => {
      expect((await fixedPoint.fraction(Q112.mul(2359).mul(
        BigNumber.from(2).pow(32),
      ), Q112.mul(50)))[0]).to.eq(
    ...
    it('can be called with numerator and denominator exceeding uint112 max', async () => {
      expect((await fixedPoint.fraction(Q112.mul(2359), Q112.mul(50)))[0])
        .to.eq(BigNumber.from(2359).mul(Q112).div(50));
    });
  ...
  describe('#reciprocal', () => {
    ...
    it('works for 0.25', async () => {
      expect((await fixedPoint.reciprocal([Q112.mul(
        BigNumber.from(25),
      ).div(100)]))[0]).to.eq(Q112.mul(4));
    });
    it('works for 5', async () => {
      expect((await fixedPoint.reciprocal([Q112.mul(
        BigNumber.from(5),
      )]))[0]).to.eq(Q112.mul(BigNumber.from(1)).div(5));
    });
  });
  ...
  describe('#sqrt', () => {
    ...
    it('works for 25', async () => {
      expect((await fixedPoint.sqrt([BigNumber.from(25)
